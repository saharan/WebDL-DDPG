 ...
 * ...
 */
class GpuReduceMeanOperation extends GpuOperation {
	var a:Tensor;
	var dst:Tensor;
	var axis:Int;
	var keepDim:Bool;
	...
	public function new(backend:GpuBackend, a:Tensor, dst:Tensor, axis:Int, keepDim:Bool) {
		...
		if (keepDim) {
			this.forwardOps = [
				fop([a], dst, "reducemean_forward_kd", '
					...
					' +
						...
						')
					+ '
						return sum / float($U_SRC_SHAPE1.$axisComp);
					}
				')
			];
			this.backwardOps = [
				bop([dst], a, "reducemean_backward_kd", '
					float run(ivec4 idx4) {
						elem dst = src1(replace$axisCompUpper(idx4, 0));
						return dst.diff / float($U_DST_SHAPE.$axisComp);
					}
				')
			];
		} else {
			this.forwardOps = [
				fop([a], dst, "reducemean_forward", '
					float run(ivec4 idx4) {
