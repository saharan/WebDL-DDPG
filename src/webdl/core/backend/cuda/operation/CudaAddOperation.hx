 ...
 * ...
 */
class CudaAddOperation extends CudaOperation {
	var a:Tensor;
	var b:Tensor;
	var dst:Tensor;
	...
	public function new(a:Tensor, b:Tensor, dst:Tensor) {
		...
		this.dst = dst;
		this.forwardOps = [
			new CudaAtomicOperation([dst, a, b], "add_forward", '
		...
		this.backwardOps = [
			new CudaAtomicOperation([broadcastedA, dst], "add_backward_a", '
				float d = dif(1, idx4);
				dif(0, idx4) += d;
			'),
			new CudaAtomicOperation([broadcastedB, dst], "add_backward_b", '
				float d = dif(1, idx4);
				dif(0, idx4) += d;
			')
		];
