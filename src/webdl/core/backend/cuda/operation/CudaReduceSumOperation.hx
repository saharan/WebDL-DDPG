 ...
 * ...
 */
class CudaReduceSumOperation extends CudaOperation {
	var a:Tensor;
	var dst:Tensor;
	var axis:Int;
	var keepDim:Bool;
	...
	public function new(a:Tensor, dst:Tensor, axis:Int, keepDim:Bool) {
		...
		if (keepDim) {
			this.forwardOps = [
				new CudaAtomicOperation([dst, a], "reducesum_forward_kd", '
					def_idx4(aIdx4);
			...
			this.backwardOps = [
				new CudaAtomicOperation([a, dst], "reducesum_backward_kd", '
					def_idx4(dstIdx4);
					dstIdx4[$axisIndex] = 0;
			...
			this.forwardOps = [
				new CudaAtomicOperation([dst, a], "reducesum_forward", '
					def_idx4(aIdx4);
			...
			this.backwardOps = [
				new CudaAtomicOperation([a, dst], "reducesum_backward", '
					def_idx4(dstIdx4);
