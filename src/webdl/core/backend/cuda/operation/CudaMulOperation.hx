 ...
 * ...
 */
class CudaMulOperation extends CudaOperation {
	var a:Tensor;
	var b:Tensor;
	var dst:Tensor;
	...
	public function new(a:Tensor, b:Tensor, dst:Tensor) {
		...
		this.dst = dst;
		this.forwardOps = [
			new CudaAtomicOperation([dst, a, b], "mul_forward", '
		...
		this.backwardOps = [
			new CudaAtomicOperation([broadcastedA, dst, b], "mul_backward_a", '
				float d = dif(1, idx4);
				float b = val(2, idx4);
				dif(0, idx4) += d * b;
			'),
			new CudaAtomicOperation([broadcastedB, dst, a], "mul_backward_b", '
				float d = dif(1, idx4);
				float a = val(2, idx4);
				dif(0, idx4) += d * a;
			')
